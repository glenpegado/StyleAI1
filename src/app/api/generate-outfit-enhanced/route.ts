...
export async function POST(request: NextRequest) {
  try {
    ...
    const enhancedOutfitData = await enhanceWithAffiliateProducts(aiOutfitData, celebrityName, styleDescription);
...
async function enhanceWithAffiliateProducts(aiOutfitData: any, celebrityName?: string, styleDescription?: string) {
  ...
  for (const category of categories) {
    ...
    for (const aiItem of aiItems) {
      try {
        ...
        let realProducts = await affiliateService.searchProducts(searchQuery, category);
        
        // If no affiliate products found, try undocumented APIs (based on Farfetch example)
        if (realProducts.length === 0) {
          console.log(`No affiliate products found for ${searchQuery}, trying undocumented APIs...`);
          const discoveredProducts = await undocumentedAPIService.discoverAllProducts(searchQuery, 10);
  ...
  if (celebrityName && styleDescription) {
    try {
      const celebrityProducts = await affiliateService.searchCelebrityStyle(celebrityName, styleDescription);
      ...
      .insert({
        ...
        products_data: {
          ...
          categories: ['tops', 'bottoms', 'accessories', 'shoes'].map(cat => ({
            ...
            real_products: outfitData[cat]?.filter((item: any) => item.real_product)?.length || 0
          }))
        }
      });
