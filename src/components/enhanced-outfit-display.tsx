      ...
      <CardContent className="p-4">
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant={
                item.availability === "In Stock" ? "default" : "secondary"
              }
              className="text-xs"
            >
              {item.availability}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {outfitData[selectedCategory]?.map((item, index) =>
          renderProductCard(item, index),
        )}
      </div>

      {/* Celebrity Alternatives */}
      {outfitData.celebrity_alternatives &&
        outfitData.celebrity_alternatives.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {outfitData.celebrity_alternatives.map((item, index) =>
                renderProductCard(item, index),
              )}
            </div>
          </div>
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="p-4">
          <p className="text-sm text-blue-800">
            ...
            through these links, at no additional cost to you. This helps
            support our platform and allows us to continue providing free
